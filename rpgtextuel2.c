...
void malusbonus(struct player *joueur)
{
    ...
    { // Si l'endurance est à 0 ou moins
...
void executer_quete(struct player *joueur, FILE *fichier)
{
    ...
    char choix[10][BUF_SIZE];             // Tableau pour stocker les 3 choix possibles
    char consequence[10][BUF_SIZE];       // Tableau pour stocker les 3 conséquences possibles

    // Lire le texte de la quête
    while (fgets(ligne, BUF_SIZE, fichier) && strstr(ligne, "===") == NULL)
    {                               // Lit jusqu'à trouver "==="
        strcat(texte_quete, ligne); // Concatène la ligne lue au texte de la quête
    }

    /*// Lire le texte de la quête
    while (fgets(ligne, BUF_SIZE, fichier) && strstr(ligne, "\n\n") == NULL)
    {                         // Lit jusqu'à trouver "==="
        strcat(choix, ligne); // Concatène la ligne lue au texte de la quête
    }

    // Lire le texte de la quête
    while (fgets(ligne, BUF_SIZE, fichier) && strstr(ligne, "\n\n") == NULL)
    {                               // Lit jusqu'à trouver "==="
        strcat(consequence, ligne); // Concatène la ligne lue au texte de la quête
    }*/

    // Lire les 3 choix
    for (int i = 0; i < 4; i++)
    { // Boucle pour lire les 3 choix
        if (!fgets(choix[i], BUF_SIZE, fichier))
    ...
    for (int i = 0; i < 4; i++)
    { // Boucle pour lire les 3 conséquences
        if (!fgets(consequence[i], BUF_SIZE, fichier))
        { // Si la lecture échoue
    ...
    {                                        // Boucle pour afficher les 3 choix
        printf("%d. %s\n", i + 0, choix[i]); // Affiche chaque choix avec un numéro
    }
    ...
    char *modif;
    while ((modif = strtok(NULL, "|")) != NULL)
    { // Parcourt les modifications de stats
        if (strstr(modif, "pv+") != NULL)
        {                                  // Si la modification concerne les points de vie (augmentation)
            joueur->pv += atoi(modif + 3); // Ajoute les points de vie (exemple : "pv+10")
        }
        else if (strstr(modif, "pv-") != NULL)
        {                                  // Si la modification concerne les points de vie (réduction)
            joueur->pv -= atoi(modif + 3); // Retire les points de vie (exemple : "pv-15")
        }
        else if (strstr(modif, "endurance+") != NULL)
        {                                          // Si la modification concerne l'endurance (augmentation)
            joueur->endurance += atoi(modif + 10); // Ajoute de l'endurance (exemple : "endurance+20")
        }
        else if (strstr(modif, "endurance-") != NULL)
        {                                          // Si la modification concerne l'endurance (réduction)
            joueur->endurance -= atoi(modif + 10); // Retire de l'endurance (exemple : "endurance-15")
        }
        else if (strstr(modif, "or+") != NULL)
        {                                  // Si la modification concerne l'or (augmentation)
            joueur->or += atoi(modif + 3); // Ajoute de l'or (exemple : "or+100")
        }
        else if (strstr(modif, "or-") != NULL)
        {                                  // Si la modification concerne l'or (réduction)
            joueur->or -= atoi(modif + 3); // Retire de l'or (exemple : "or-15")
        }
        else if (strstr(modif, "event+") != NULL)
        {                                     // Si la modification concerne l'événement (augmentation)
...
int main()
{
    ...
    if (choix == 'o' || choix == 'O')
    {                     // Si le joueur choisit de charger
